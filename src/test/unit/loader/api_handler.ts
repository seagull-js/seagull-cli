...
class ApiHandlerTest {
  ...
  'can load an API handler directly from file'() {
    const file = join(__dirname, 'example', 'Hello.js')
    ...
    expect(handler.name).to.include('example-Hello')
    expect(handler.module).to.be.a('function')
    expect(handler.module.path).to.be.equal('/hello')
    expect(handler.module.method).to.be.equal('GET')
    expect(handler.handler).to.include('Hello.handler')
  }
}
